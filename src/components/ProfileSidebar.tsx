...
const SIDEBAR_WIDTH = width * 0.75; // Reduced to 75%
...
export const ProfileSidebar = ({ visible, onClose }: ProfileSidebarProps) => {
    ...
    const { darkMode, setDarkMode, language, setLanguage, translations: t, isLoadingTranslations } = useSettings();
    ...
    const menuItems = [
        { icon: 'person-outline', label: t.editProfile, screen: 'EditProfile', color: 'bg-blue-500', iconColor: '#fff' },
        { icon: 'globe-outline', label: t.language, screen: null, color: 'bg-purple-500', iconColor: '#fff' },
        { icon: 'settings-outline', label: t.settings, screen: 'Settings', color: 'bg-gray-500', iconColor: '#fff' },
    ];
                    ...
                    <View className="flex-1 px-6 pt-8">
                        ...
                        <View className="gap-4">
                                    ...
                                    return (
                                        <React.Fragment key={index}>
                                            ...
                                            >
                                                ...
                                                <Ionicons name={expandedLanguage ? "chevron-down" : "chevron-forward"} size={16} color={darkMode ? "#4b5563" : "#9ca3af"} />
                                            </TouchableOpacity>
                                            ...
                                            {expandedLanguage && (
                                                <View className="pl-14 gap-2 mb-2">
                                                    <TouchableOpacity
                                                        className={`flex-row items-center py-2 ${language === 'en' ? (darkMode ? 'bg-green-900/30' : 'bg-green-50') : ''} rounded-lg px-2`}
                                                        onPress={() => setLanguage('en')}
                                                    >
                                                        <Text className={`flex-1 ${language === 'en' ? 'text-green-500 font-bold' : (darkMode ? 'text-gray-400' : 'text-gray-600')}`}>English</Text>
                                                        {language === 'en' && <Ionicons name="checkmark" size={16} color="#22c55e" />}
                                                    </TouchableOpacity>
                                                    <TouchableOpacity
                                                        className={`flex-row items-center py-2 ${language === 'tl' ? (darkMode ? 'bg-green-900/30' : 'bg-green-50') : ''} rounded-lg px-2`}
                                                        onPress={() => setLanguage('tl')}
                                                    >
                                                        <Text className={`flex-1 ${language === 'tl' ? 'text-green-500 font-bold' : (darkMode ? 'text-gray-400' : 'text-gray-600')}`}>Tagalog</Text>
                                                        {language === 'tl' && <Ionicons name="checkmark" size={16} color="#22c55e" />}
                                                    </TouchableOpacity>
                                                </View>
                                            )}
                                        </React.Fragment>
                            ...
                            >
                                ...
                                <Text className={`text-base font-medium mr-20 ${darkMode ? "text-gray-200" : "text-gray-700"}`}>{t.darkMode}</Text>
